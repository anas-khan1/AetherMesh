...
import { nodeList, selectedNodeDefault, sidebarNavItems, throughputHistory, storageDistribution, recentActivity } from '../data/mockData';
...
function StoragePie() {
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.5 }} className="panel p-5">
      ...
      <ResponsiveContainer width="100%" height={180}>
        <PieChart>
          <Pie data={storageDistribution} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={3} dataKey="value" startAngle={90} endAngle={-270}>
            {storageDistribution.map((entry, i) => (<Cell key={i} fill={entry.color} stroke="none" />))}
          </Pie>
          <Tooltip contentStyle={{ background: 'rgba(12,30,55,0.95)', border: '1px solid rgba(17,232,246,0.2)', borderRadius: '6px', fontSize: '11px', fontFamily: 'var(--font-mono)' }} />
        </PieChart>
      </ResponsiveContainer>
      <div className="flex flex-wrap items-center justify-center gap-3 mt-2">
        {storageDistribution.map((s) => (
        ...
        <main className="flex flex-col gap-5 min-w-0">
          ...
          >
            ...
            <div className="status-pill">
              ...
              128 of 130 nodes online
            </div>
          </motion.header>

          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <MetricCard label="Total Throughput" value="1.42" unit="GB/s" delay={0} trend="up" />
            <MetricCard label="Active Nodes" value="128" suffix="/ 130" delay={100} trend="stable" />
            <MetricCard label="Mesh Latency" value="12.4" unit="ms" delay={200} trend="down" />
            <MetricCard label="Data Resilience" value="99.99" suffix="%" delay={300} trend="stable" />
          </div>
